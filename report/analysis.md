...
实验现象：ROP$^{+}$ 相比输入的 $\Delta\sigma_Y=0.1878$，$\Delta H=1.1422$，恢复后 $d_{\mathrm{rgb}}=0.0099$。这些数值可用于报告中描述可见度、信息量和色偏变化。
...
图片内容：该图包含退化输入、ROP 输出、ROP$^{+}$ 输出、两种散射图热力可视化以及 $3|J-I|$ 差异图。前三幅给出主观视觉质量，后三幅解释算法内部估计与像素变化区域。
绘图逻辑：C++ 输出恢复图与中间散射图，Python 按论文常见的子图编号方式排成 2x3 面板。散射热力图越偏暖，表示估计的介质散射/传输修正越强。
实验现象：ROP$^{+}$ 相比输入的 $\Delta\sigma_Y=0.1600$，$\Delta H=0.9676$，恢复后 $d_{\mathrm{rgb}}=0.0724$。这些数值可用于报告中描述可见度、信息量和色偏变化。
理论分析：若 ROP$^{+}$ 的散射图比 ROP 更贴合远景和低对比区域，说明统一光谱筛选与 TV 细化减少了近景纹理对散射估计的干扰。若差异图在强边缘处明显，通常代表恢复公式主要增强结构边界；若在平坦区域大面积发亮，则可能存在过校正。
...
图片内容：该图包含退化输入、ROP 输出、ROP$^{+}$ 输出、两种散射图热力可视化以及 $3|J-I|$ 差异图。前三幅给出主观视觉质量，后三幅解释算法内部估计与像素变化区域。
绘图逻辑：C++ 输出恢复图与中间散射图，Python 按论文常见的子图编号方式排成 2x3 面板。散射热力图越偏暖，表示估计的介质散射/传输修正越强。
实验现象：ROP$^{+}$ 相比输入的 $\Delta\sigma_Y=0.1627$，$\Delta H=0.9313$，恢复后 $d_{\mathrm{rgb}}=0.0623$。这些数值可用于报告中描述可见度、信息量和色偏变化。
理论分析：若 ROP$^{+}$ 的散射图比 ROP 更贴合远景和低对比区域，说明统一光谱筛选与 TV 细化减少了近景纹理对散射估计的干扰。若差异图在强边缘处明显，通常代表恢复公式主要增强结构边界；若在平坦区域大面积发亮，则可能存在过校正。
...
图片内容：该图包含退化输入、ROP 输出、ROP$^{+}$ 输出、两种散射图热力可视化以及 $3|J-I|$ 差异图。前三幅给出主观视觉质量，后三幅解释算法内部估计与像素变化区域。
绘图逻辑：C++ 输出恢复图与中间散射图，Python 按论文常见的子图编号方式排成 2x3 面板。散射热力图越偏暖，表示估计的介质散射/传输修正越强。
实验现象：ROP$^{+}$ 相比输入的 $\Delta\sigma_Y=0.1185$，$\Delta H=0.4687$，恢复后 $d_{\mathrm{rgb}}=0.0323$。这些数值可用于报告中描述可见度、信息量和色偏变化。
理论分析：若 ROP$^{+}$ 的散射图比 ROP 更贴合远景和低对比区域，说明统一光谱筛选与 TV 细化减少了近景纹理对散射估计的干扰。若差异图在强边缘处明显，通常代表恢复公式主要增强结构边界；若在平坦区域大面积发亮，则可能存在过校正。
...
图片内容：该图包含退化输入、ROP 输出、ROP$^{+}$ 输出、两种散射图热力可视化以及 $3|J-I|$ 差异图。前三幅给出主观视觉质量，后三幅解释算法内部估计与像素变化区域。
绘图逻辑：C++ 输出恢复图与中间散射图，Python 按论文常见的子图编号方式排成 2x3 面板。散射热力图越偏暖，表示估计的介质散射/传输修正越强。
实验现象：ROP$^{+}$ 相比输入的 $\Delta\sigma_Y=0.0313$，$\Delta H=0.0969$，恢复后 $d_{\mathrm{rgb}}=0.1283$。这些数值可用于报告中描述可见度、信息量和色偏变化。
理论分析：若 ROP$^{+}$ 的散射图比 ROP 更贴合远景和低对比区域，说明统一光谱筛选与 TV 细化减少了近景纹理对散射估计的干扰。若差异图在强边缘处明显，通常代表恢复公式主要增强结构边界；若在平坦区域大面积发亮，则可能存在过校正。
...
图片内容：该图包含退化输入、ROP 输出、ROP$^{+}$ 输出、两种散射图热力可视化以及 $3|J-I|$ 差异图。前三幅给出主观视觉质量，后三幅解释算法内部估计与像素变化区域。
绘图逻辑：C++ 输出恢复图与中间散射图，Python 按论文常见的子图编号方式排成 2x3 面板。散射热力图越偏暖，表示估计的介质散射/传输修正越强。
实验现象：ROP$^{+}$ 相比输入的 $\Delta\sigma_Y=0.1031$，$\Delta H=0.5217$，恢复后 $d_{\mathrm{rgb}}=0.0289$。这些数值可用于报告中描述可见度、信息量和色偏变化。
理论分析：若 ROP$^{+}$ 的散射图比 ROP 更贴合远景和低对比区域，说明统一光谱筛选与 TV 细化减少了近景纹理对散射估计的干扰。若差异图在强边缘处明显，通常代表恢复公式主要增强结构边界；若在平坦区域大面积发亮，则可能存在过校正。
...
图片内容：该图包含退化输入、ROP 输出、ROP$^{+}$ 输出、两种散射图热力可视化以及 $3|J-I|$ 差异图。前三幅给出主观视觉质量，后三幅解释算法内部估计与像素变化区域。
绘图逻辑：C++ 输出恢复图与中间散射图，Python 按论文常见的子图编号方式排成 2x3 面板。散射热力图越偏暖，表示估计的介质散射/传输修正越强。
实验现象：ROP$^{+}$ 相比输入的 $\Delta\sigma_Y=0.0556$，$\Delta H=0.4157$，恢复后 $d_{\mathrm{rgb}}=0.1888$。这些数值可用于报告中描述可见度、信息量和色偏变化。
理论分析：若 ROP$^{+}$ 的散射图比 ROP 更贴合远景和低对比区域，说明统一光谱筛选与 TV 细化减少了近景纹理对散射估计的干扰。若差异图在强边缘处明显，通常代表恢复公式主要增强结构边界；若在平坦区域大面积发亮，则可能存在过校正。
